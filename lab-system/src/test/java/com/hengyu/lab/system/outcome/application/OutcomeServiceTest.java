...
class OutcomeServiceTest {
  ...
  void test_delete_outcome_fail(){
      Mockito.when(outcomeRepository.findById(1L)).thenThrow(new BizException(OutcomeResultCode.OUTCOME_NOT_FOUND));
      Assertions.assertThatThrownBy(() -> outcomeService.deleteOutcome(1L)).isInstanceOf(BizException.class);
